...
export function logRemoteBundleInfo(logger: Logger, remoteBundle: RemoteBundleInfo, byteLength: number): void {
  const { name: bundleName, version, etag, integrity, signature, lastModified } = remoteBundle;
  logger.info(`Remote Webview Bundle: ${c.info(bundleName)} ${c.bytes(formatByteLength(byteLength))}`);
