...
pub fn setup_logging(level: LoggingLevel, kind: LoggingKind) {
...
/// - All spans and events at level debug in crates whose name starts with `webview-bundle`
struct LoggingFilter {
  level: LoggingLevel,
}

/// Tracing filter used for spans emitted by `webview-bundle*` crates
...
impl LoggingFilter {
  fn check_enabled(&self, meta: &Metadata<'_>) -> bool {
    let filter = if meta.target().starts_with("webview_bundle") {
...
impl<S> Filter<S> for LoggingFilter {
  fn enabled(&self, meta: &Metadata<'_>, _: &Context<'_, S>) -> bool {
    self.check_enabled(meta)
  }

  fn callsite_enabled(&self, meta: &'static Metadata<'static>) -> Interest {
    if self.check_enabled(meta) {
      Interest::always()
    } else {
      Interest::never()
    }
